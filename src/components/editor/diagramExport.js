...
function contentBounds(shapes, shapeOrder, arrows, arrowOrder) {
  ...
  for (const id of arrowOrder) {
    ...
    // Locked or not, a label's position is always labelAnchor + its manual
    // offset (see ArrowLabels.jsx) - locking only stops the offset itself
    // from being dragged to a new value, it doesn't change where the label
    // renders relative to the arrow's own live route.
